...
class Game5_Win4 : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_game5_win4)
        ...
        findViewById<Button>(R.id.next_game).setOnClickListener {
            lifecycleScope.launch(Dispatchers.IO) {
                ...
                lifecycleScope.launch(Dispatchers.Main) {
                    Log.i("devl|game54", "Moving to the next game")

                    if (next_game != null)
                        Utilities.startActivity(this@Game5_Win4, next_game)
                    else
                        Utilities.startActivity(this@Game5_Win4, LandingActivity::class.java)
