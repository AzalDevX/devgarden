...
class Game1_Win4 : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_game1_win5)
        ...
        findViewById<Button>(R.id.next_game).setOnClickListener {
            lifecycleScope.launch(Dispatchers.IO) {
                ...
                lifecycleScope.launch(Dispatchers.Main) {
                    Log.i("devl|game44", "Moving to the next game")

                    if (next_game != null)
                        Utilities.startActivity(this@Game1_Win4, next_game)
                    else
                        Utilities.startActivity(this@Game1_Win4, LandingActivity::class.java)
